...
void main() {
  ...
  testWidgets('MobileCartFloatingButton renders and CartPage displays full order breakdown', (tester) async {
    CartController.addItem(testProduct);

    await tester.pumpWidget(_wrapTestWidget(const CartPage()));
    await tester.pump();

    // Verify CartPage elements
  ...
  testWidgets('Cart component renders cleanly in standalone/drawer mode', (tester) async {
    CartController.addItem(testProduct);

    await tester.pumpWidget(
      _wrapTestWidget(
        const Scaffold(
          body: Cart(isDrawerMode: true),
        ),
      ),
    );
    await tester.pump();
