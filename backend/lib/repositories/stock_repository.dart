...
class StockRepository {
  ...
  Future<StockRow> create({
    ...
    required String storeId,
      ...
      .insertValue(productId: productId, storeId: storeId)
      .returnInserted()
      .executeAndFetch();
  ...
  Future<StockRow?> update({
    ...
    if (transactionType == StockTransactionType.reduce.name) {
      if (stockRow.quantity <= 0) {
        throw ArgumentError(
          'Cannot reduce stock because current inventory is 0 units.',
        );
      }
      if (amount != null && amount > stockRow.quantity) {
        throw ArgumentError(
          'Reduction amount ($amount) exceeds available stock (${stockRow.quantity} units).',
        );
      }
    }
