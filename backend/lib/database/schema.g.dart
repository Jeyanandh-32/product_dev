...
final class _$CustomerRecentStoresRow extends CustomerRecentStoresRow {
  _$CustomerRecentStoresRow._(
    this.customerId,
    this.storeId,
    this.lastVisitedAt,
  );
  ...
  static final _$table = $ForGeneratedCode.tableDefinition(
    tableName: 'customer_recent_stores',
    columns: <String>['customer_id', 'store_id', 'last_visited_at'],
    columnInfo: [
      $ForGeneratedCode.columnDefinition(
        type: $ForGeneratedCode.text,
        isNotNull: true,
  ...
  static CustomerRecentStoresRow? _$fromDatabase(RowReader row) {
    final customerId = row.readString();
    final storeId = row.readString();
    final lastVisitedAt = row.readDateTime();
    if (customerId == null && storeId == null && lastVisitedAt == null) {
      return null;
    }
    return _$CustomerRecentStoresRow._(customerId!, storeId!, lastVisitedAt!);
  }

  @override
  String toString() =>
      'CustomerRecentStoresRow(customerId: "$customerId", storeId: "$storeId", lastVisitedAt: "$lastVisitedAt")';
}

/// Extension methods for table defined in [CustomerRecentStoresRow].
extension TableCustomerRecentStoresRowExt on Table<CustomerRecentStoresRow> {
  /// Insert row into the `customerRecentStores` table.
  ///
  /// Returns a [InsertSingle] statement on which `.execute` must be
  /// called for the row to be inserted.
  InsertSingle<CustomerRecentStoresRow> insert({
    required Expr<String> customerId,
    required Expr<String> storeId,
    Expr<DateTime>? lastVisitedAt,
  }) => $ForGeneratedCode.insertInto(
    table: this,
    values: [customerId, storeId, lastVisitedAt],
  );
  ...
  /// Returns a [InsertSingle] statement on which `.execute` must be
  /// called for the row to be inserted.
  InsertSingle<CustomerRecentStoresRow> insertValue({
    required String customerId,
    required String storeId,
    DateTime? lastVisitedAt,
  }) => $ForGeneratedCode.insertInto(
    table: this,
    values: [customerId.asExpr, storeId.asExpr, lastVisitedAt?.asExpr],
  );

  /// Bulk insert rows into the `customerRecentStores` table.
  ///
  /// This method takes an `Iterable<T>` and requires that you provide
  /// a _mapping function_ from `T` to each column to be inserted.
  ///
  ...
  /// Returns a [Insert] statement on which `.execute` must be
  /// called for the rows to be inserted.
  Insert<CustomerRecentStoresRow> insertValuesMapped<T>(
    Iterable<T> rows, {
    required String Function(T row) customerId,
    required String Function(T row) storeId,
    DateTime Function(T row)? lastVisitedAt,
  }) => $ForGeneratedCode.insertValuesMapped(
    table: this,
    rows: rows,
    mappings: [customerId, storeId, lastVisitedAt],
  );

  /// Delete a single row from the `customerRecentStores` table, specified by
  /// _primary key_.
  ///
  /// Returns a [DeleteSingle] statement on which `.execute()` must be
  /// called for the row to be deleted.
  ...
  /// should be deleted. If you wish to delete all rows, use
  /// `.where((_) => toExpr(true)).delete()`.
  DeleteSingle<CustomerRecentStoresRow> delete(
    String customerId,
    String storeId,
  ) => $ForGeneratedCode.deleteSingle(
...
/// Extension methods for building queries against the `customerRecentStores` table.
extension QueryCustomerRecentStoresRowExt
    on Query<(Expr<CustomerRecentStoresRow>,)> {
  /// Lookup a single row in `customerRecentStores` table using the _primary key_.
  ///
  /// Returns a [QuerySingle] object, which returns at-most one row,
  /// when `.fetch()` is called.
  QuerySingle<(Expr<CustomerRecentStoresRow>,)> byKey(
    String customerId,
    String storeId,
  ) => where(
    (customerRecentStoresRow) =>
        customerRecentStoresRow.customerId.equalsValue(customerId) &
        customerRecentStoresRow.storeId.equalsValue(storeId),
  ).first;

  /// Update all rows in the `customerRecentStores` table matching this [Query].
  ///
  /// The changes to be applied to each row matching this [Query] are
  /// defined using the [updateBuilder], which is given an [Expr]
  /// representation of the row being updated and a `set` function to
  ...
  /// > the `set` function more than once, and the result should always
  /// > be returned immediately.
  Update<CustomerRecentStoresRow> update(
    UpdateSet<CustomerRecentStoresRow> Function(
      Expr<CustomerRecentStoresRow> customerRecentStoresRow,
      UpdateSet<CustomerRecentStoresRow> Function({
    ...
    updateBuilder,
  ) => $ForGeneratedCode.update<CustomerRecentStoresRow>(
    this,
    _$CustomerRecentStoresRow._$table,
    (customerRecentStoresRow) => updateBuilder(
      customerRecentStoresRow,
      ({
        Expr<String>? customerId,
        Expr<String>? storeId,
        Expr<DateTime>? lastVisitedAt,
      }) => $ForGeneratedCode.buildUpdate<CustomerRecentStoresRow>([
        customerId,
        storeId,
        lastVisitedAt,
      ]),
  ...
  /// Delete all rows in the `customerRecentStores` table matching this [Query].
  ///
  /// Returns a [Delete] statement on which `.execute()` must be called
  /// for the rows to be deleted.
  Delete<CustomerRecentStoresRow> delete() =>
      $ForGeneratedCode.delete(this, _$CustomerRecentStoresRow._$table);
}

/// Extension methods for building point queries against the `customerRecentStores` table.
extension QuerySingleCustomerRecentStoresRowExt
    on QuerySingle<(Expr<CustomerRecentStoresRow>,)> {
  /// Update the row (if any) in the `customerRecentStores` table matching this
  /// [QuerySingle].
  ///
  /// The changes to be applied to the row matching this [QuerySingle] are
  /// defined using the [updateBuilder], which is given an [Expr]
  ...
  /// > the `set` function more than once, and the result should always
  /// > be returned immediately.
  UpdateSingle<CustomerRecentStoresRow> update(
    UpdateSet<CustomerRecentStoresRow> Function(
      Expr<CustomerRecentStoresRow> customerRecentStoresRow,
      UpdateSet<CustomerRecentStoresRow> Function({
        Expr<String> customerId,
    ...
    updateBuilder,
  ) => $ForGeneratedCode.updateSingle<CustomerRecentStoresRow>(
    this,
    _$CustomerRecentStoresRow._$table,
    (customerRecentStoresRow) => updateBuilder(
      customerRecentStoresRow,
  ...
  /// Delete the row (if any) in the `customerRecentStores` table matching this [QuerySingle].
  ///
  /// Returns a [DeleteSingle] statement on which `.execute()` must be called
  /// for the row to be deleted. The resulting statement will **not**
  /// fail, if there are no rows matching this query exists.
  DeleteSingle<CustomerRecentStoresRow> delete() =>
      $ForGeneratedCode.deleteSingle(this, _$CustomerRecentStoresRow._$table);
}

/// Extension methods for expressions on a row in the `customerRecentStores` table.
extension ExpressionCustomerRecentStoresRowExt
    on Expr<CustomerRecentStoresRow> {
  ...
  Expr<DateTime> get lastVisitedAt =>
      $ForGeneratedCode.field(this, 2, $ForGeneratedCode.dateTime);
}

extension ExpressionNullableCustomerRecentStoresRowExt
    on Expr<CustomerRecentStoresRow?> {
  Expr<String?> get customerId =>
      $ForGeneratedCode.field(this, 0, $ForGeneratedCode.text);
...
/// `Table<CustomerRecentStoresRow>` conflict targets for use with `.onConflict`.
enum CustomerRecentStoresRowConflict {
  /// Conflict with an existing row that has a matching primary key.
  ///
  /// Thus, the other row has matching values for:
  /// `customerId`, `storeId`.
  ...
  const CustomerRecentStoresRowConflict(this._fields);

  final List<String> _fields;
}

extension InsertCustomerRecentStoresRowExt on Insert<CustomerRecentStoresRow> {
  /// Build an `INSERT` statement with an `ON CONFLICT` clause.
  ///
  /// The [target] argument specifies the _conflict target_ to be
  /// handled. The _conflict target_ is always a `UNIQUE` constraint or
  ...
  /// then the conflict action is triggered:
  /// * `.doNothing()` to skip insertion of the new row, and,
  /// * `.update((customerRecentStoresRow, excluded, set) => set(...))` to
  ///   update the conflicting row.
  ///
  /// If a row to be inserted violates a constraint other than the one
  /// specified in _conflict target_ then the entire `INSERT` statement
  ...
  ///
  /// This is equivalent to `INSERT ... ON CONFLICT (...)` in SQL.
  InsertOnConflict<CustomerRecentStoresRow> onConflict(
    CustomerRecentStoresRowConflict target,
  ) => $ForGeneratedCode.insertOnConflict(this, target._fields);
}

extension InsertOnConflictCustomerRecentStoresRowExt
    on InsertOnConflict<CustomerRecentStoresRow> {
  /// Build an `INSERT` statement an [upsert-clause][1].
  ///
  /// When a row to be inserted violates the `UNIQUE` or `PRIMARY KEY`
  /// constraint previously specified as _conflict target_, the existing
  /// row is updated using the expressions defined with the
  /// [updateBuilder]. The [updateBuilder] is given 3 parameters:
  ///   * `customerRecentStoresRow` an [Expr] representing the existing row in
  ///     the database,
  ///   * `excluded` an [Expr] representing the row to be inserted in the
  ///     database, and,
  ///   * `set` a function to specify which fields should be updated and
  ...
  ///
  /// [1]: https://www.sqlite.org/lang_upsert.html
  Upsert<CustomerRecentStoresRow> update(
    UpdateSet<CustomerRecentStoresRow> Function(
      Expr<CustomerRecentStoresRow> customerRecentStoresRow,
      Expr<CustomerRecentStoresRow> excluded,
      UpdateSet<CustomerRecentStoresRow> Function({
        Expr<String> customerId,
        Expr<String> storeId,
        Expr<DateTime> lastVisitedAt,
    ...
    updateBuilder,
  ) => $ForGeneratedCode.updateOnConflict<CustomerRecentStoresRow>(
    this,
    (customerRecentStoresRow, excluded) => updateBuilder(
      customerRecentStoresRow,
      excluded,
      ({
        Expr<String>? customerId,
...
extension InsertSingleCustomerRecentStoresRowExt
    on InsertSingle<CustomerRecentStoresRow> {
  /// Build an `INSERT` statement with an `ON CONFLICT` clause.
  ///
  /// The [target] argument specifies the _conflict target_ to be
  /// handled. The _conflict target_ is always a `UNIQUE` constraint or
  ...
  ///
  /// This is equivalent to `INSERT ... ON CONFLICT (...)` in SQL.
  InsertOnConflictSingle<CustomerRecentStoresRow> onConflict(
    CustomerRecentStoresRowConflict target,
  ) => $ForGeneratedCode.insertOnConflictSingle(this, target._fields);
}

extension InsertOnConflictSingleCustomerRecentStoresRowExt
    on InsertOnConflictSingle<CustomerRecentStoresRow> {
  /// Build an `INSERT` statement an [upsert-clause][1].
  ///
  /// When a row to be inserted violates the `UNIQUE` or `PRIMARY KEY`
  /// constraint previously specified as _conflict target_, the existing
  ...
  ///
  /// [1]: https://www.sqlite.org/lang_upsert.html
  UpsertSingle<CustomerRecentStoresRow> update(
    UpdateSet<CustomerRecentStoresRow> Function(
      Expr<CustomerRecentStoresRow> customerRecentStoresRow,
      Expr<CustomerRecentStoresRow> excluded,
      UpdateSet<CustomerRecentStoresRow> Function({
    ...
    updateBuilder,
  ) => $ForGeneratedCode.updateOnConflictSingle<CustomerRecentStoresRow>(
    this,
    (customerRecentStoresRow, excluded) => updateBuilder(
      customerRecentStoresRow,
      excluded,
