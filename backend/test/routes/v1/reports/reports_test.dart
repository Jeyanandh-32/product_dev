...
void main() {
  ...
  test('/v1/reports/dashboard GET responds with dashboard analytics map', () async {
    when(() => request.method).thenReturn(.get);
    when(() => request.uri).thenReturn(Uri.parse('http://localhost/v1/reports/dashboard?storeId=$validStoreId'));
    when(() => orderRepo.getDashboardAnalytics(merchantId: 'm-1', storeId: validStoreId, fromDate: any(named: 'fromDate'), toDate: any(named: 'toDate')))
        .thenAnswer((_) async => {'totalRevenue': 10000.0, 'totalOrders': 5, 'topProducts': <Map<String, dynamic>>[]});
  ...
  test('/v1/reports/payments GET responds with payment collection breakdown', () async {
    when(() => request.method).thenReturn(.get);
    when(() => request.uri).thenReturn(Uri.parse('http://localhost/v1/reports/payments?storeId=$validStoreId'));
    when(() => orderRepo.count(merchantId: 'm-1', storeId: validStoreId, fromDate: any(named: 'fromDate'), toDate: any(named: 'toDate'), paymentMethod: any(named: 'paymentMethod'), paymentStatus: any(named: 'paymentStatus')))
        .thenAnswer((_) async => 5);
    when(() => orderRepo.getAll(merchantId: 'm-1', storeId: validStoreId, fromDate: any(named: 'fromDate'), toDate: any(named: 'toDate'), paymentMethod: any(named: 'paymentMethod'), paymentStatus: any(named: 'paymentStatus'), limit: any(named: 'limit'), offset: any(named: 'offset')))
        .thenAnswer((_) async => []);
    when(() => orderRepo.getPaymentSummary(merchantId: 'm-1', storeId: validStoreId, fromDate: any(named: 'fromDate'), toDate: any(named: 'toDate')))
        .thenAnswer((_) async => (cashCollected: 3000.0, upiCollected: 2000.0, freeTotal: 0.0, totalCollected: 5000.0));
  ...
  test('/v1/reports/profit-loss GET responds with profit & loss breakdown', () async {
    when(() => request.method).thenReturn(.get);
    when(() => request.uri).thenReturn(Uri.parse('http://localhost/v1/reports/profit-loss?storeId=$validStoreId'));
    when(() => orderRepo.getProfitLossReport(merchantId: 'm-1', storeId: validStoreId, fromDate: any(named: 'fromDate'), toDate: any(named: 'toDate'), searchQuery: any(named: 'searchQuery'), limit: any(named: 'limit'), offset: any(named: 'offset')))
        .thenAnswer((_) async => (total: 0, items: <ProfitLossItem>[], totalCostPrice: 30000.0, totalCollectedPrice: 50000.0, totalProfit: 20000.0, totalMarginPercentage: 40.0));
