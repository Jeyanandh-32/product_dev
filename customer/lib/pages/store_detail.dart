...
class _StoreDetailPageState extends SignalState<StoreDetailPage> {
  final storeSignal = asyncSignal<Store?>(const AsyncLoading());
  ...
  Component buildSignal(BuildContext context) {
    ...
    if (storeState.isLoading || productsState.isLoading || categoriesState.isLoading) {
      return div(
        classes: 'flex-1 min-h-[60vh] flex flex-col items-center justify-center gap-4 text-center my-auto w-full',
        [
          span(classes: 'loading loading-spinner loading-lg text-black', []),
          p(classes: 'text-sm font-semibold text-gray-500', [.text('Loading store menu...')]),
        ],
      );
    }

    final store = storeState.value;
    if (storeState.hasError || store == null) {
      return div(
        classes: 'min-h-[50vh] flex flex-col items-center justify-center gap-4 text-center px-4',
        [
          h1(classes: 'text-2xl font-bold text-black', [.text('Store Not Found')]),
          p(classes: 'text-sm text-gray-500', [.text('The requested store catalog does not exist or is inactive.')]),
        ],
      );
    }

    final cartItems = storeCartsSignal.value[store.id] ?? {};
    ...
    return div(
      ...
      [
        ...
        div(classes: 'flex flex-col gap-3', [
          StoreProductSearchBar(searchQuery: _searchQuery, onQueryChanged: (val) => setState(() => _searchQuery = val)),
        ...
        div(
          ...
          [h2(classes: 'text-xl font-extrabold text-black tracking-tight', [.text(_searchQuery.isEmpty ? 'Menu Products' : 'Search Results')])],
        ),
        ...
        FloatingCartBar(totalCartCount: totalCartCount, totalCartPrice: totalCartPrice),
      ],
    );
  }
}
